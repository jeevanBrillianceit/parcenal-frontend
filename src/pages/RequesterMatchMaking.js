...
const RequesterMatchMaking = () => {
  ...
  const handleSendRequest = async (receiverId, index, contextType, contextId) => {
    ...
    try {
      setResults(prev => prev.map((item, i) => i === index ? { ...item, isSending: true } : item));
      ...
      if (res.data.status === 1) {
        ...
        setResults(prev => prev.map((item, i) => i === index ? { ...item, isSending: false } : item));
      }
    } catch (err) {
  ...
  const handleOutsideClick = (e) => {
    if (!dropdownRefs.departure.current?.contains(e.target) && !dropdownRefs.arrival.current?.contains(e.target)) {
      setFocusedDropdown('');
    }
  };
  ...
  const handleSearch = async () => {
    ...
    try {
      setLoading(true);
      const res = await axiosInstance.post('/user/match-requesters', payload);
      if (res.data.status === 1) {
      ...
      <ul className="absolute z-10 bg-white border w-full max-h-52 overflow-y-auto">
        {filteredAirports[type].map((a, i) => {
          ...
          return (
            <li
              key={a.icao + i}
              className={`px-3 py-2 cursor-pointer hover:bg-gray-200 ${highlightedIndex[type] === i ? 'bg-blue-100' : ''}`}
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center px-4 py-10">
      <div className="bg-white shadow-md rounded p-6 w-full max-w-2xl mb-8">
        <h2 className="text-2xl font-bold mb-6 text-center">Requester Matchmaking</h2>
        ...
        <label className="block font-bold mb-1">Arrival Date</label>
        <input
          type="date"
          value={selectedDate}
...
export default RequesterMatchMaking;